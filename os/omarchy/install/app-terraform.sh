...
if check_pacman_package "terraform"; then
    install_pacman "terraform"
else
    info "Installing from AUR..."
    install_yay "terraform"
fi
...
if [[ -d "/usr/share/bash-completion/completions" ]]; then
    ...
    if $BINARY -help 2>/dev/null | grep -q "autocomplete"; then
        ...
        # The autocomplete install creates a file, let's move it to standard location
        if [[ -f "/etc/bash_completion.d/terraform" ]]; then
            sudo mv /etc/bash_completion.d/terraform /usr/share/bash-completion/completions/terraform 2>/dev/null || true
            success "Completions installed to /usr/share/bash-completion/completions/terraform"
        fi
    fi
elif [[ -d "/etc/bash_completion.d" ]]; then
    if $BINARY -help 2>/dev/null | grep -q "autocomplete"; then
        $BINARY -install-autocomplete 2>/dev/null || true
        success "Completions installed to /etc/bash_completion.d/terraform"
    fi
else
    # User-local

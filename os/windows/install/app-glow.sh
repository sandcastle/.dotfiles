...
case "$ENV" in
    git-bash)
        ...
        if command -v winget &> /dev/null; then
            install_winget "Charmbracelet.Glow"
        else
            error "winget not found. Please install winget or download glow from:"
            info "https://github.com/charmbracelet/glow/releases/latest"
            exit 1
        fi
        ;;
    wsl)
        info "$APP_NAME - A markdown renderer for the terminal"
        info "Installing in WSL via apt repository..."
        
        # Use Charm repository for apt

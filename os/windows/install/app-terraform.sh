...
case "$ENV" in
    git-bash)
        ...
        if command -v winget &> /dev/null; then
            install_winget "Hashicorp.Terraform"
        else
            error "winget not found. Please install winget or download Terraform from:"
            info "https://developer.hashicorp.com/terraform/install"
            exit 1
        fi
        ;;
    wsl)
